...
{
	const SDIO_BOOT_OPTIONS_T *ptSdioOptions;
...
//				trace_message_ul(TRACEMSG_SDIO_Error, ulValue);
		...
		{
//			trace_message_ul(TRACEMSG_SDIO_CommandTimeOut, ulTimeoutUs);
...
static SDIO_RESULT_T sdio_check_capacity(SDIO_HANDLE_T *ptSdioHandle, const TEST_PARAMETER_T *ptTestParameter)
{
	SDIO_RESULT_T tResult;
	union CSD_REGISTER_UNION
	{
	...
	unsigned long ulCapacityKbMin;
	unsigned long ulCapacityKbMax;
...
//			ullCapacity = ((unsigned long long)uiCSize) * ((unsigned long long)(uiBlockLen*uiMult));
		...
		if( tResult==SDIO_RESULT_Ok )
		{
			ulCapacityKbMin = ptTestParameter->ulCapacityKbMin;
			ulCapacityKbMax = ptTestParameter->ulCapacityKbMax;

			uprintf("Capacity: %dKB\n", ulSizeKB);
			uprintf("Minimum: ");
			if( ulCapacityKbMin==0U )
			{
				uprintf("none\n");
			}
			else
			...
			if( ulCapacityKbMin!=0U && ulSizeKB<ulCapacityKbMin )
			{
				uprintf("The capacity is below the accepted minimum.\n");
				tResult = SDIO_RESULT_InvalidCapacity;
			}
			else if( ulCapacityKbMax!=0U && ulSizeKB>ulCapacityKbMax )
			{
				uprintf("The capacity is above the accepted maximum.\n");
				tResult = SDIO_RESULT_InvalidCapacity;
			}
			else
			{
				uprintf("The capacity is in the accepted range.\n");
				tResult = SDIO_RESULT_Ok;
			}
		}
	}
...
static SDIO_RESULT_T sdio_initialize(SDIO_HANDLE_T *ptSdioHandle, const SDIO_BOOT_OPTIONS_T *ptSdioOptions, const TEST_PARAMETER_T *ptTestParameter)
{
	HOSTDEF(ptRAPSDIOArea);
	unsigned long ulValue;
					...
					if( ptSdioHandle->uiCardIsMMC==0 )
					{
						...
						if( tResult==SDIO_RESULT_Ok )
						{
							...
							if( tResult==SDIO_RESULT_Ok )
							{
								...
								if( tResult==SDIO_RESULT_Ok )
								{
									tResult = sdio_check_capacity(ptSdioHandle, ptTestParameter);
									if( tResult==SDIO_RESULT_Ok )
									{
...
static int sdio_enable_power(const SDIO_BOOT_OPTIONS_T *ptSdioOptions)
{
...
int boot_sdio(const SDIO_BOOT_OPTIONS_T *ptSdioOptions, const TEST_PARAMETER_T *ptTestParameter)
{
	...
	if( iResult==0 )
	{
		...
		else
		{
			...
			tResult = sdio_initialize(&tSdioHandle, ptSdioOptions, ptTestParameter);
			if( tResult!=SDIO_RESULT_Ok )
			{
...
//	portcontrol_restore(ausPortcontrol_Index_SDIO, sizeof(ausPortcontrol_Index_SDIO)/sizeof(ausPortcontrol_Index_SDIO[0]));
